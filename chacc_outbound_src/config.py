...
def get_outbound_config(module_context) -> dict:
    ...
    if module_context is not None:
        port_str = module_context.get_module_config("EMAIL_SMTP_PORT", "chacc_outbound", default="587")
        try:
            port = int(port_str)
        except (TypeError, ValueError):
            port = 587
        use_tls_raw = module_context.get_module_config("EMAIL_SMTP_USE_TLS", "chacc_outbound", default=None)
